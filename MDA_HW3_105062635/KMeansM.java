...
public class KMeansM {
		...
		public void reduce(Text key, Iterable<Text> values,	Context context	) throws IOException, InterruptedException {	
			...
			for(String c : clusters){
				...
				for(int i=0; i<b.length; i++){
					distance += Math.abs(Double.parseDouble(b[i]) - Double.parseDouble(a[i+1]));
				}
				if(distance < min){
					min = distance;
	...
	public static void main(String[] args) throws Exception {
		...
		if (otherArgs.length != 3) {
			System.err.println("Usage: KMeansM <in1> <in2> <out>");
			System.exit(2);
		}
		...
		for(int i=0; i<MAX_ITER; i++){
			Job job1 = new Job(conf, "KMeansM step 1");
			job1.setJarByClass(KMeansM.class);
			...
			Job job2 = new Job(conf, "KMeansM step 2");
			job2.setJarByClass(KMeansM.class);
		...
		for(int i=0; i<pList.size(); i++){
			for(int j=0; j<pList.size(); j++){
				...
				for(int k=0; k<DIM; k++){
					distance += Math.abs(Double.parseDouble(b[k]) - Double.parseDouble(a[k]));
				}
				System.out.print(Double.toString(distance) + " ");
			}
			System.out.print("\n");
		}
		in.close();
